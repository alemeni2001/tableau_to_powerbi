...
def generate_json_line_graph(extracted_data, name):
    """
    Genera la estructura JSON para un visual de Power BI tipo línea (lineChart) con jerarquía de fechas en el eje X.
    """
    for worksheet_data in extracted_data:
        ...
        worksheet_cols = worksheet["cols"].strip("[]") if worksheet["cols"] else ""
        worksheet_rows = worksheet["rows"].strip("[]") if worksheet["rows"] else ""
        ...
        # Ajusta estos valores si tu campo de fecha tiene otro nombre
        date_property = worksheet_cols or "Fecha"
        value_property = worksheet_rows or "venta"
        hierarchy_name = "Jerarquía de fechas"
        hierarchy_level = "Día"
        property_variation_name = "Variación"
                            ...
                            "projections": [
                                {
                                    ...
                                    "queryRef": f"{worksheet_relation}.{date_property}.{property_variation_name}.{hierarchy_name}.{hierarchy_level}",
                                    "nativeQueryRef": f"{date_property} {hierarchy_level}",
                                    "active": True
                                }
                            ]
                        },
                            ...
                            "projections": [
                                {
                                    ...
                                    "queryRef": f"Sum({worksheet_relation}.{value_property})",
                                    "nativeQueryRef": f"Suma de {value_property}"
                                }
                            ]
                        }
                    },
    ...
    return data
def generate_json_graph(extracted_data, name):
    """
    Genera la estructura JSON para un visual de Power BI a partir de los datos extraídos de un worksheet.
    Devuelve el diccionario listo para ser guardado como visual.json.
    """
    graph_data = []
    for worksheet_data in extracted_data:
        worksheet_title = worksheet_data["worksheet"]["worksheet_title"]
        worksheet_relation = worksheet_data["worksheet"]["worksheet_datasources"][0].get('caption', '')
        dependency_columns = worksheet_data["worksheet"]["dependency_info"][0]["columns"]
        worksheet_cols = worksheet_data["worksheet"]["cols"].strip("[]") if worksheet_data["worksheet"]["cols"] else ""
        worksheet_rows = worksheet_data["worksheet"]["rows"].strip("[]") if worksheet_data["worksheet"]["rows"] else ""
        worksheet_visualType = worksheet_data["worksheet"]["type"]
        worksheet_derivation = {
            "Sum": 0, "Promedio": 1, "Recuento(distintivo)": 2, "Minimo": 3, "Maximo": 4,
            "Recuento": 5, "Mediana": 6, "DesviacionEstándar": 7, "Varianza": 8, "None": 0
        }.get(worksheet_data["worksheet"]["dependency_info"][0]["column_instances"][0].get("derivation", ""), 0)
        direction = "Ascending"
        isDefaultSort_true = True
...
if __name__ == "__main__":
    ...
    # Puedes agregar una carpeta extra dentro de base_path si quieres
    output_folder = os.path.join(base_path, "output", "Demo.report", "definition")
    ...
    for dashboard in dashboards:
        ...
        page_folder = os.path.join(output_folder, "pages", dashboard_hex)
        ...
        for worksheet_name in dashboard["worksheet_names"]:
            ...
            if worksheet_data:
                ...
                if not os.path.exists(visual_json_path):
                    ...
                    data = generate_json_line_graph([worksheet_data], worksheet_hex)
    ...
    pages_json_path = os.path.join(output_folder, "pages", "pages.json")
    ...
    report_json_path = os.path.join(output_folder, "report.json")
